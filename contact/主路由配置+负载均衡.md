...
将路由器接到lan口，设置路由器为DHCP模式，网关和DNS设置为openwrt的管理页（此处是192.168.3.100）的IP地址，连接到wifi在浏览器输入192.168.3.100进入openwrt管理页
...
在openwrt管理页，网络服务-接口，关闭“忽略此接口”，高级中开启动态DHCP和强制DHCP服务，若是宿主机操作openwrt管理页，此时将路由器接到lan口，设置路由器为DHCP模式，路由器的网关和DNS设置为openwrt的管理页（此处是192.168.3.100）的IP地址路
...
```shell
sudo docker network create -d macvlan --subnet=10.117.30.0/24 --gateway=10.117.30.1 -o parent=enp0s20f0u5u4c2 macnet 
```
...
进入opwrt管理页，添加接口名为"wan"（只能这个名字）接口，网卡选择macnet2
